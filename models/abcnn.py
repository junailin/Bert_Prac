...
class Abcnn3(nn.Module):
    ...
    def dropout(self, v):
        return F.dropout(v, p=self.dropout_value)

    # x1:       torch.Size([32, 1, 15, 200])        (batch_size, 1, seq_len, word_emb_length)
    # output:   torch.Size([32, 1])                 (batch_size, 1)
    def forward(self, x1, x2):
        # print('[model-forward] x1:', x1.size())
        '''
...
class InceptionModule(nn.Module):
    ...
    def __init__(self, in_channel, sentence_length, filter_width, filter_height, filter_channel):
        ...
        self.conv_2 = convolution(in_channel, filter_width + 4, filter_height, int(filter_channel / 3),
                                  filter_width + 1)
        conv_3_padding = int((sentence_length + filter_width - 2) / 2)
        self.conv_3 = convolution(in_channel, sentence_length, filter_heightp, int(filter_channel / 3),
                                  conv_3_padding)
    ...
    def forward(self, x):
        ...
        out_2 = self.conv_2(x)
        out_3 = self.conv_3(x)
        # print('[InceptionModule-forward] x:', x.size())  #  [15, 2, 20, 768]
        # print('[InceptionModule-forward] out_1:', out_1.size())  # [15, 44, 20, 1] ; [9, 43, 15, 1]
        # print('[InceptionModule-forward] out_2:', out_2.size())  # [15, 43, 20, 1]
        # print('[InceptionModule-forward] out_3:', out_3.size())  # [15, 43, 19, 1]
