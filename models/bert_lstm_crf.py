...
class BertLstmCrf(BertPreTrainedModel):
    def __init__(self, config, tagset_size):  # 这个config是bert的config
        ...
        if self.bilstm_flag:
            lstm_hidden = self.lstm_hidden_size // 2
        else:
        ...
        if self.word_feature_extractor == "GRU":
            ...
            self.lstm = nn.LSTM(self.input_size, lstm_hidden, num_layers=self.lstm_layer, batch_first=True,
                                bidirectional=self.bilstm_flag)
        self.hidden2tag = nn.Linear(self.lstm_hidden_size, self.tagset_size)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask=None, label_ids=None):
        # ----- 词嵌入层 -----
        word_represent, _ = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        word_represent = self.dropout(word_represent)
        word_represent = word_represent.transpose(1, 0)
        ...
        lstm_outputs = self.hidden2tag(feature_out)

        # ----- crf 层 -----
        outs = lstm_outputs
        batch_size = outs.size(0)
        # loss计算
        total_loss = self.crf.neg_log_likelihood_loss(outs, attention_mask, label_ids)
        if self.average_batch:
            total_loss = total_loss / batch_size
        # 维特比解码，求最优tag_seq
        scores, tag_seq = self.crf._viterbi_decode(outs, attention_mask)
        return total_loss, scores, tag_seq